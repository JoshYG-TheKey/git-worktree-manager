...
from git_worktree_manager.cli import GitWorktreeCLI, cli, main
...
class TestGitWorktreeCLI:
    ...
    def test_validate_git_repository_failure(self, mock_console, mock_git_ops):
        ...
        # Setup mock
        mock_git_instance = Mock()
        mock_git_instance.is_git_repository.side_effect = GitRepositoryError(
...
class TestCLICommands:
    ...
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_create_command_basic_functionality(
        self, mock_validate, mock_worktree_manager
    ):
    ...
    @patch("git_worktree_manager.worktree_manager.WorktreeManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_list_command_basic_functionality(
        self, mock_validate, mock_worktree_manager
    ):
    ...
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_configure_command_basic_functionality(
        self, mock_validate, mock_config_manager
    ):
...
class TestMainFunction:
    ...
    def test_main_unexpected_exception(self, mock_console, mock_cli):
        ...
        assert result == 1
        mock_console.print.assert_called_once_with(
            "[red]Unexpected error:[/red] Unexpected error"
        )
...
class TestCLIIntegration:
    ...
    def test_full_cli_workflow_valid_repo(self, mock_git_ops, mock_worktree_manager):
        ...
        with patch("git_worktree_manager.config.ConfigManager") as mock_config_manager:
            mock_config_instance = Mock()
            mock_config_instance.get_default_worktree_location.return_value = (
                "/test/path"
            )
            mock_config_instance.load_user_preferences.return_value = {}
            mock_config_manager.return_value = mock_config_instance
...
class TestCreateCommand:
    ...
    @patch("git_worktree_manager.worktree_manager.WorktreeManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_create_command_worktree_creation_error(
        self, mock_validate, mock_worktree_manager
    ):
        ...
        mock_validate.return_value = True
        mock_manager_instance = Mock()
        mock_manager_instance.create_worktree.side_effect = WorktreeCreationError(
            "Creation failed"
        )
        mock_worktree_manager.return_value = mock_manager_instance
    ...
    @patch("git_worktree_manager.worktree_manager.WorktreeManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_create_command_keyboard_interrupt(
        self, mock_validate, mock_worktree_manager
    ):
    ...
    @patch("git_worktree_manager.worktree_manager.WorktreeManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_create_command_unexpected_error(
        self, mock_validate, mock_worktree_manager
    ):
        ...
        mock_validate.return_value = True
        mock_manager_instance = Mock()
        mock_manager_instance.create_worktree.side_effect = Exception(
            "Unexpected error"
        )
        mock_worktree_manager.return_value = mock_manager_instance
...
class TestListCommand:
    ...
    def test_list_command_with_diff(self, mock_validate, mock_worktree_manager):
        ...
        from git_worktree_manager.models import DiffSummary, WorktreeInfo
    ...
    def test_list_command_git_error(self, mock_validate, mock_worktree_manager):
        ...
        mock_validate.return_value = True
        mock_manager_instance = Mock()
        mock_manager_instance.list_worktrees.side_effect = GitRepositoryError(
            "Git error"
        )
        mock_worktree_manager.return_value = mock_manager_instance
    ...
    @patch("git_worktree_manager.worktree_manager.WorktreeManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_list_command_keyboard_interrupt(
        self, mock_validate, mock_worktree_manager
    ):
...
class TestConfigureCommand:
    ...
    @patch("git_worktree_manager.config.ConfigManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_configure_command_reset_confirmed(
        self, mock_validate, mock_config_manager
    ):
    ...
    @patch("git_worktree_manager.config.ConfigManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_configure_command_reset_cancelled(
        self, mock_validate, mock_config_manager
    ):
    ...
    @patch("git_worktree_manager.config.ConfigManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_configure_command_interactive_no_change(
        self, mock_validate, mock_config_manager
    ):
    ...
    @patch("git_worktree_manager.config.ConfigManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_configure_command_interactive_change_location(
        self, mock_validate, mock_config_manager, mock_mkdir, mock_exists
    ):
    ...
    def test_configure_command_config_error(self, mock_validate, mock_config_manager):
        ...
        mock_validate.return_value = True
        mock_config_instance = Mock()
        mock_config_instance.load_user_preferences.side_effect = ConfigError(
            "Config error"
        )
        mock_config_manager.return_value = mock_config_instance
    ...
    @patch("git_worktree_manager.config.ConfigManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_configure_command_keyboard_interrupt(
        self, mock_validate, mock_config_manager
    ):
    ...
    @patch("git_worktree_manager.config.ConfigManager")
    @patch("git_worktree_manager.cli.GitWorktreeCLI.validate_git_repository")
    def test_configure_command_unexpected_error(
        self, mock_validate, mock_config_manager
    ):
        ...
        mock_validate.return_value = True
        mock_config_instance = Mock()
        mock_config_instance.get_default_worktree_location.side_effect = Exception(
            "Unexpected error"
        )
        mock_config_manager.return_value = mock_config_instance
