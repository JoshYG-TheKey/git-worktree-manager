...
class TestWithRetryDecorator:
    ...
    def test_max_delay_cap(self, mock_sleep):
        ...
        @with_retry(
            RetryConfig(
                max_attempts=4, base_delay=1.0, max_delay=2.5, backoff_multiplier=2.0
            )
        )
        def failing_operation():
            nonlocal call_count
...
class TestWorktreeCleanupManager:
    ...
    def test_cleanup_failed_worktree(self, mock_exists, mock_rmtree, mock_run):
        ...
        assert worktree_remove_call[0][0] == [
            "git",
            "worktree",
            "remove",
            "--force",
    ...
    @patch("shutil.rmtree")
    @patch("os.path.exists")
    def test_cleanup_failed_worktree_with_errors(
        self, mock_exists, mock_rmtree, mock_run
    ):
...
class TestErrorRecoveryManager:
    ...
    def test_safe_worktree_creation_success(self):
        ...
        result = manager.safe_worktree_creation(
            creation_func, "/test/worktree", "test-branch"
        )
    ...
    def test_safe_git_operation_non_transient_error(self, mock_check_git):
        ...
        def git_operation():
            ...
            raise subprocess.CalledProcessError(
                1, ["git", "test"], stderr=b"fatal error"
            )

        with pytest.raises(
            GitRepositoryError, match="Git operation 'test_operation' failed"
        ):
            manager.safe_git_operation(git_operation, "test_operation")
...
class TestIntegrationScenarios:
    ...
    @patch("shutil.rmtree")
    @patch("os.path.exists")
    def test_complete_worktree_creation_failure_recovery(
        self, mock_exists, mock_rmtree, mock_run
    ):
        ...
        def failing_creation():
            ...
            raise subprocess.CalledProcessError(
                128, ["git", "worktree", "add"], stderr=b"worktree creation failed"
            )

        with pytest.raises(WorktreeCreationError):
            manager.safe_worktree_creation(
                failing_creation, "/test/worktree", "test-branch"
            )
