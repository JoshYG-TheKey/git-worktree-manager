...
class TestConfigValidation:
    ...
    def test_validate_worktree_config_empty_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ConfigValidationError, match="default_path cannot be empty"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_worktree_config_non_string_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ConfigValidationError, match="default_path must be a string"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_worktree_config_relative_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ConfigValidationError, match="must be absolute or start with ~"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_worktree_config_non_bool_cleanup(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config = Config(
                worktree=WorktreeConfig(default_path="/test", auto_cleanup="yes")
            )

            with pytest.raises(
                ConfigValidationError, match="auto_cleanup must be a boolean"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_ui_config_non_bool_progress(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ConfigValidationError, match="show_progress must be a boolean"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_performance_config_non_int_timeout(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ConfigValidationError, match="cache_timeout must be an integer"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_performance_config_negative_timeout(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ConfigValidationError, match="cache_timeout must be non-negative"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_performance_config_excessive_timeout(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ConfigValidationError, match="cannot exceed 86400 seconds"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_performance_config_non_int_max_items(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ConfigValidationError, match="max_cached_items must be an integer"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_performance_config_zero_max_items(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(
                ConfigValidationError, match="max_cached_items must be at least 1"
            ):
                config_manager.validate_config(config)
    ...
    def test_validate_and_save_preferences_invalid_setting(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            prefs = {"worktree": {"unknown_setting": "value"}}
