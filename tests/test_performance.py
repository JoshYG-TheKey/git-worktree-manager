...
from git_worktree_manager.models import DiffSummary
...
class TestGitOperationsPerformance:
    ...
    def test_numstat_parsing_performance(self):
        ...
        assert result_numstat.files_modified + result_numstat.files_added + result_numstat.files_deleted > 0
        assert result_stat.files_modified + result_stat.files_added + result_stat.files_deleted > 0
    ...
    def test_progressive_loading_performance(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            start_time = time.time()
            diff_limited = self.git_ops.get_diff_summary_progressive("main", "dev", max_files=100)
            ...
            start_time = time.time()
            diff_unlimited = self.git_ops.get_diff_summary_progressive("main", "dev", max_files=None)
            ...
            limited_total = diff_limited.files_modified + diff_limited.files_added + diff_limited.files_deleted
            unlimited_total = diff_unlimited.files_modified + diff_unlimited.files_added + diff_unlimited.files_deleted
...
class TestRealRepositoryPerformance:
    ...
    def test_large_repository_simulation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            subprocess.run(["git", "checkout", "-b", "feature"], cwd=temp_dir, check=True, capture_output=True)
            ...
            subprocess.run(["git", "add", "."], cwd=temp_dir, check=True, capture_output=True)
            subprocess.run(["git", "commit", "-m", "Feature changes"], cwd=temp_dir, check=True, capture_output=True)
            ...
            total_files_changed = (diff_summary.files_modified + 
            ...
            start_time = time.time()
            diff_progressive = git_ops.get_diff_summary_progressive("main", "feature", max_files=10)
            ...
            assert progressive_time <= max(diff_time * 1.5, 0.1)  # Allow 50% margin or 100ms max
