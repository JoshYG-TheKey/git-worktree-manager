...
class WorktreeCleanupManager:
    ...
    def execute_cleanup(self, operation_id: str) -> None:
        ...
        for cleanup_func in reversed(cleanup_functions):  # Execute in reverse order
            try:
                cleanup_func()
                logger.debug(
                    f"Successfully executed cleanup function for {operation_id}"
                )
            except Exception as e:
    ...
    def cleanup_failed_worktree(
        self, worktree_path: str, branch_name: Optional[str] = None
    ) -> None:
        """
        ...
        try:
            ...
            logger.warning(
                f"Timeout while removing Git worktree tracking for: {worktree_path}"
            )
        except Exception as e:
            logger.warning(
                f"Failed to remove Git worktree tracking for {worktree_path}: {e}"
            )
        ...
        try:
            ...
            if branch_name:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    logger.info(
                        f"Branch {branch_name} exists but cleanup is conservative - manual review recommended"
                    )
        except Exception as e:
            logger.warning(f"Failed to check/cleanup branch {branch_name}: {e}")

        logger.info(
            f"Completed worktree cleanup for {worktree_path}. Steps attempted: {cleanup_steps}"
        )
...
class GracefulDegradationManager:
    ...
    def check_feature_availability(
        self, feature_name: str, check_func: Callable
    ) -> bool:
        """
    ...
    def execute_with_fallback(
        ...
        try:
            ...
            if feature_name in self.fallback_strategies:
                logger.warning(
                    f"Primary function failed for {feature_name}: {e}. Using fallback."
                )
        ...
        try:
            ...
            if cleanup_on_failure:
                self.cleanup_manager.register_cleanup(
                    operation_id,
                    lambda: self.cleanup_manager.cleanup_failed_worktree(
...
def get_error_recovery_manager(repo_path: str = ".") -> ErrorRecoveryManager:
    ...
    if (
        _error_recovery_manager is None
        or _error_recovery_manager.repo_path != repo_path
    ):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            return recovery_manager.safe_worktree_creation(
                lambda: func(*args, **kwargs), worktree_path, branch_name
            )
