...
class UIController:
    ...
    def select_base_branch(self, branches: List[str], current_branch: Optional[str] = None) -> str:
        ...
        while True:
            try:
                ...
                if 1 <= choice <= len(branches):
                    ...
                    self.console.print(f"[success]Selected:[/success] [branch]{selected_branch}[/branch]")
                    return selected_branch
                else:
                    self.display_error(f"Please enter a number between 1 and {len(branches)}")
    ...
    def select_worktree_location(self, default_path: str = "") -> str:
        ...
        def validate_path(path: str) -> str:
            ...
            if os.path.exists(abs_path):
                if os.path.isfile(abs_path):
                    raise ValueError(f"Path [path]{abs_path}[/path] is a file, not a directory")
                elif os.path.isdir(abs_path) and os.listdir(abs_path):
                    raise ValueError(f"Directory [path]{abs_path}[/path] already exists and is not empty")
        ...
        while True:
            try:
                ...
                self.console.print(f"[success]Location:[/success] [path]{validated_path}[/path]")
    ...
    def display_worktree_list(self, worktrees: List[WorktreeInfo]) -> None:
        ...
        for worktree in worktrees:
            ...
            # Format commit info
            commit_display = worktree.commit_hash[:8] if worktree.commit_hash else "unknown"
    ...
    def display_worktree_details(self, worktree: WorktreeInfo) -> None:
        ...
        if worktree.base_branch:
            details_content.append(f"Base Branch: [branch]{worktree.base_branch}[/branch]")
        ...
        panel = Panel(
            panel_content,
            title=f"[branch]Worktree Details[/branch]",
            border_style="blue",
            box=box.ROUNDED,
            padding=(1, 2)
        )
    ...
    def display_diff_summary(self, diff_summary: DiffSummary, worktree_branch: str = "", base_branch: str = "") -> None:
        ...
        total_files = diff_summary.files_modified + diff_summary.files_added + diff_summary.files_deleted
        if total_files == 0:
            ...
            if diff_summary.files_modified > 0:
                file_stats.append(f"[modified]{diff_summary.files_modified} modified[/modified]")
            if diff_summary.files_deleted > 0:
                file_stats.append(f"[deleted]{diff_summary.files_deleted} deleted[/deleted]")
    ...
    def display_diff_visualization(self, diff_summary: DiffSummary, max_width: int = 50) -> None:
        ...
        if additions_width > 0:
            viz_content.append(f"Additions ({diff_summary.total_insertions}): {additions_bar}")
        if deletions_width > 0:
            viz_content.append(f"Deletions ({diff_summary.total_deletions}): {deletions_bar}")
    ...
    def display_file_change_indicators(self, files_added: int, files_modified: int, files_deleted: int) -> str:
        ...
        return " ".join(indicators) if indicators else "[unchanged]no changes[/unchanged]"