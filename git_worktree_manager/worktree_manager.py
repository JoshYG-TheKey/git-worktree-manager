...
class WorktreeManager:
    ...
    def create_worktree(self, 
                       ...
                       location: Optional[str] = None) -> WorktreeInfo:
        ...
        try:
            ...
            if location is None:
                try:
                    ...
                    location = self.ui_controller.select_worktree_location(default_location)
                except KeyboardInterrupt:
                    self.ui_controller.stop_progress()
                    raise WorktreeCreationError("Operation cancelled by user")
            ...
            # Create the worktree
            try:
                self.git_ops.create_worktree(location, branch_name, base_branch)
            except GitRepositoryError as e:
                self.ui_controller.stop_progress()
            ...
            try:
                worktree_info = self._get_worktree_info(location, branch_name, base_branch)
            ...
            self.ui_controller.stop_progress()
            raise WorktreeCreationError(f"Unexpected error during worktree creation: {e}")
    ...
    def calculate_diff_summary(self, 
                             worktree: WorktreeInfo,
                             base_branch: Optional[str] = None) -> Optional[DiffSummary]:
        ...
        try:
                ...
                try:
                    ...
                    if current_branch != worktree.branch:
                        ...
                        for fallback in ['main', 'master', 'develop']:
                            if fallback in available_branches and fallback != worktree.branch:
            ...
            try:
                diff_summary = self.git_ops.get_diff_summary(base_branch, worktree.branch)
    ...
    def _validate_worktree_creation_inputs(self, 
                                         branch_name: str, 
                                         base_branch: str, 
                                         location: str) -> None:
        ...
        try:
            ...
            if location_path.exists() and location_path.is_file():
                raise WorktreeCreationError(f"Location is a file, not a directory: {location}")
            
            if location_path.exists() and location_path.is_dir() and any(location_path.iterdir()):
                raise WorktreeCreationError(f"Location already exists and is not empty: {location}")
    ...
    def _get_worktree_info(self, 
                          location: str, 
                          branch_name: str, 
                          base_branch: str) -> WorktreeInfo:
